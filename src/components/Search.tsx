...
const Search: React.FC = () => {
  ...
  useEffect(() => {
    const handleOutsideClick = (e: MouseEvent) => {
      if (searchContainerRef.current && !searchContainerRef.current.contains(e.target as Node)) {
        setSuggestions([]);
        setShowInput(false); // Close the input field when clicking outside
      }
    };
  ...
  const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (keyword) {
      ...
      setShowInput(false); // Close the input field after searching
    }
  };
  ...
  const handleSuggestionClick = (suggestion: Product) => {
    ...
    setShowInput(false); // Close the input field after selecting a suggestion
  };
    ...
    <>
      <div className="relative flex justify-center flex-wrap gap-5">
        <div className="w-full max-w-md relative" ref={searchContainerRef}>
          {!showInput ? (
            <Button onClick={toggleInput} variant="outline">
              <GoSearch />
            </Button>
          ) : (
          ...
          {showInput && keyword && suggestions.length > 0 && (
